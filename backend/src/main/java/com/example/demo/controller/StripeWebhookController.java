...
public class StripeWebhookController {
        ...
        switch (event.getType()) {
            case "checkout.session.completed" -> {
                if (objeto instanceof Session session) {
                    Long barbeariaId = extrairBarbeariaId(session.getMetadata());
                    PlanoSaas plano = extrairPlano(session.getMetadata());
                    if (barbeariaId != null) {
                        assinaturaService.confirmarCheckout(
                                session.getCustomer(), session.getSubscription(), barbeariaId, plano);
                    }
                }
            }
    ...
    private Long extrairBarbeariaId(java.util.Map<String, String> metadata) {
        if (metadata == null || metadata.get("barbeariaId") == null) return null;
        try {
            return Long.parseLong(metadata.get("barbeariaId"));
